...
Prompt.prototype.onKeypress = function( s, key ) {
  ...
  this.opt.choices.choices = this.choices.filter(function(choice){
    return choice instanceof Separator || choice.value.toLowerCase().indexOf(line) >= 0;
  });
  this.opt.choices.realChoices = this.opt.choices.filter(Separator.exclude);
  ...
  if( key.name === "tab") {
    this.rl.line = '';
    this.opt.choices.choices = this.choices;
    this.opt.choices.realChoices = this.opt.choices.filter(Separator.exclude);
    this.pointer = 0;
  }
